...
local function UpdateAuras(self, event, unit)
	...
	if(element) then
		...
		local visibleBuffs, hiddenBuffs = filterIcons(element, unit, element.buffFilter or element.filter or 'HELPFUL', min(numBuffs, max), nil, 0, true)

		local visibleDebuffs, hiddenDebuffs = filterIcons(element, unit, element.buffFilter or element.filter or 'HARMFUL', min(numDebuffs, max - visibleBuffs), true, visibleBuffs)

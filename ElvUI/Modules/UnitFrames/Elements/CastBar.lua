...
function UF:PostCastStart(unit)
	...
	if self.channeling and db.castbar.ticks and unit == "player" then
		...
		if baseTicks and unitframe.ChannelTicksSize[self.spellName] and unitframe.HastedChannelTicks[self.spellName] then
			...
			local baseTickSize = unitframe.ChannelTicksSize[self.spellName]
			...
			self.hadTicks = true
		elseif baseTicks and unitframe.ChannelTicksSize[self.spellName] then
			local curHaste = UnitSpellHaste("player") * 0.01
			local baseTickSize = unitframe.ChannelTicksSize[self.spellName]
			local hastedTickSize = baseTickSize / (1 +  curHaste)

...
function UF:Configure_Castbar(frame)
	...
	local customTextFont = db.customTextFont
	local customTimeFont = db.customTimeFont

	if customTextFont.enable then
		castbar.Text:FontTemplate(LSM:Fetch("font", customTextFont.font), customTextFont.fontSize, customTextFont.fontStyle)
	else
	...
	castbar.Text:SetShown(not db.hidetext)

	if customTimeFont.enable then
		castbar.Time:FontTemplate(LSM:Fetch("font", customTimeFont.font), customTimeFont.fontSize, customTimeFont.fontStyle)
	else
	...
	castbar.Time:SetShown(not db.hidetext)
...
function UF:PostCastStart(unit)
	...
	if self.channeling and db.castbar.ticks and unit == "player" then
		...
		local ticksSize = baseTicks and unitframe.ChannelTicksSize[self.spellName]
		local hasteTicks = ticksSize and unitframe.HastedChannelTicks[self.spellName]
